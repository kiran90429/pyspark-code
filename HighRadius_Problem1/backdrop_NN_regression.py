...
#     Z[Z < 0] = 0
    ...
    #In case of Classification I need to code it to sigmoid function;
    #expA = np.exp(A)
...
def cost(T, Y):
    #regression cost function is (T - Y) * (T - Y);
    
    tot_cost = (T - Y).T.dot(T - Y);
    return tot_cost;
...
def main():
    ...
    #filling null in place of NA
    data.fillna(0,inplace=True);
    
    df = data[data["isOpen"]==0];
    
    
    #excluding the null elements in the dates;
    ...
    #formating the due date and update
    ...
    df["final_delay"] = [(datetime.datetime.strptime(str(start),"%Y%m%d") - datetime.datetime.strptime(str(end),"%Y%m%d")).days for start,end in zip(df["update_date"],df["due_date"])]   
        
    df["invoice_amount_grouped"] = 0;
    
    #df_final["invoice_amount"] = df_final["invoice_amount"].astype(int);
    
    amount_labels = np.arange(10,210);
    
    #bin_values = np.arange(-150000,300000,250);
    
    df["invoice_amount_grouped"] = pd.qcut(df["invoice_amount"],200,labels=amount_labels)
    ...
    df_predict = df["final_delay"];
    
    
    print df_predict;
    
    
    df_predict_std = (df_predict - df_predict.mean())/df_predict.std();
       
    label_mean = df_predict.mean();
    
    label_std = df_predict.std();
    ...
    due_date_t = pd.to_datetime(df["due_date"]);
    print df_features;
    
    
    X = np.array(df_features);
    
    T = np.array(df_predict_std);
   
    T = T.reshape(T.shape[0],1)
    
    K = 1;
    
    M = 6; # Same Accuracy for 4 neural networks also.
    ...
    df_test["due_date"] = df_test["due_date"].apply(lambda x:datetime.datetime.strptime(x,"%Y-%m-%d").strftime("%Y%m%d"));
    
    
    df_test["update_date"] = df_test["update_date"].apply(lambda x:datetime.datetime.strptime(x,"%Y-%m-%d").strftime("%Y%m%d"));
    
    
    
    df_test["final_delay"] = [(datetime.datetime.strptime(str(start),"%Y%m%d") - datetime.datetime.strptime(str(end),"%Y%m%d")).days for start,end in zip(df_test["update_date"],df_test["due_date"])]   
...
#     
#     df_test["invoice_amount_grouped"] = pd.cut(df_test["invoice_amount"],bins=20,labels=amount_labels)
#     
...
#     
    ...
    #invoice_amount_grouped
    df_test_final = df_test[["reference","invoice_amount","ship_to","payment_terms","month","week","quarter"]];
    ...
    for epoch in xrange(100000):
        ...
        if epoch % 100 == 0:
            ...
            delay_predicted = ((output2*label_std) + (label_mean));
            
            
            
            x = delay_predicted.reshape(delay_predicted.shape[0]);
            
            
            #delay_predicted = delay_predicted.apply(lambda x:float(x));
            
            Ypredicted_date = [start + datetime.timedelta(end) for start,end in zip(due_date_t,x)]
            
            print Ypredicted_date;
            
            #df_predicted_clearing_date_t = [start + datetime.timedelta(end) for start,end in zip(due_date_t,delay_predicted)];
    
            #Ypredicted_date = [start + datetime.timedelta(end) for start,end in zip(due_date_t,pd.DataFrame(delay_predicted))]
